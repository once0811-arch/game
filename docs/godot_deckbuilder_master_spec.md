...
- 초반은 주인공 혼자 버티지만, 중간 보스 이후 첫 동료를 반드시 영입한다.
- Act 1 보스 처치 후 두 번째 동료를 얻어 게임의 핵심 조합이 빠르게 열린다.

Act 2: 뒤틀린 탐사로
- 폐광, 뒤틀린 숲길, 학회 임시 연구소 잔해, 상단 물자 창고 폐허.
- 두 동료와의 시너지가 본격화된다.
- 중간 지점에서는 강화 타이밍이 오고, Act 2 보스 후에는 주인공 또는 동료 중 하나를 선택해 강화한다.
...
Act 1 보스 후 두 번째 동료를 누구로 받을지에 따라 장비, 카드 보상, 상점 구매가 달라졌다.
```
...
따라서 Act 1 중간 보스와 Act 1 보스에서 누구를 영입하느냐는 다음 의미를 갖는다.

```txt
첫 동료 영입 = 게임의 핵심 전술 리듬과 동료 카드풀이 처음 열린다.
두 번째 동료 영입 = 전투 인원 증가 + 카드풀 증가 + 새 서약 전술 + 장비 슬롯 증가.
Act 2 강화 선택 = 새 동료가 아니라, 이미 만든 조합을 주인공/동료 중 어디에 밀어줄지 결정한다.
```
...
동료는 이 게임의 핵심 재미이므로 Act 1 보스 후까지 미루지 않는다.

Act 1 중간 지점에는 필수 **중간 보스/계약 노드**를 배치하고, 승리 후 첫 동료를 반드시 영입한다. 이 노드는 일반 보스처럼 긴 전투가 아니라, Act 1 초반 학습을 마친 플레이어에게 동료 시스템을 여는 강한 엘리트급 시험이다.

```txt
Act 1 depth 1~5: 주인공 단독 전투와 기본 덱 학습.
Act 1 depth 6: 중간 보스/계약 노드, 승리 후 첫 동료 3택.
Act 1 depth 7~12: 첫 동료, 서약 전술, 동료 카드 보상 학습.
Act 1 보스 후: 두 번째 동료 3택.
Act 2 중간 지점: 강화 타이밍.
Act 2 보스 후: 주인공 또는 보유 동료 중 하나를 선택해 강화.
```
...
Act 1 중간 보스 처치 후 첫 동료 3택은 "드디어 우리 게임이 열린다"는 감각을 준다.
Act 1 보스 처치 후 두 번째 동료 3택은 "이번 덱에 누가 맞는가"로 고민하게 한다.
...
동료별 패시브: 장기 목표. 현재 구현은 서약 전술 + 유대 보너스 중심
동료별 서약 전술: 3개
동료별 전용 카드: 최종 목표 8장, 현재 플레이 가능 구현 3장
...
Act 1 중간 보스 후 첫 동료 영입
Act 1 보스 후 두 번째 동료 영입
Act 2부터는 신규 동료보다 주인공/보유 동료 강화 선택
```
...
현재 Godot 빌드는 동료 10명과 각 3개 서약 전술을 영입 후보로 제공한다. 로완, 세라, 엘드릭은 첫 플레이 감각을 검증하는 기준점으로 유지한다.
...
| 마렌 | 방어/약한 회복 수도승 | 조용히 상처를 돌보는 순례자. | 회색 천 조각과 약초 냄새 |
...
| 카일 | 판돈/상점 보상형 | 늘 웃지만 손익 계산이 빠른 거래꾼. | 은화가 든 찢어진 주머니 |
...
동료 카드 강화: 해당 동료 +5
장비를 착용한 동료가 전투 승리: 해당 동료 +1
Act 2 중간 강화 노드에서 동료 선택: 해당 동료 +10
Act 2 보스 후 동료 강화 선택: 해당 동료 +20
```
...
기본 공격력: 4
핵심 키워드: 약한 회복, 방어, 피해 감소, 안정성
```

### 패시브: 조용한 간호

```txt
기본: 전투 종료 시 체력 2 회복. 이번 전투에서 스킬 카드를 6장 이상 사용했다면 추가로 1 회복.
유대 60: 기본 회복 3, 조건부 추가 회복 1.
유대 100: 엘리트 전투 승리 시 추가로 체력 2 회복.
```
...
상처 봉합: 체력이 50% 이하일 때 회복 카드를 사용하면 추가로 방어도 4 획득. 턴당 1회, 전투당 2회.
...
| 상처 돌보기 | 스킬 | 2 | 체력 5 회복, 방어도 8. 강화: 회복 7, 방어도 11. |
| 침착한 호흡 | 스킬 | 0 | 방어도 4. 카드 1장 드로우. 강화: 방어도 6. |
| 고통 나누기 | 스킬 | 1 | 이번 턴 받는 피해 25% 감소. 강화: 35% 감소. |
| 순례자의 인내 | 파워 | 1 | 전투 중 처음 회복할 때 방어도 6 획득. 강화: 방어도 9. |
| 수도원의 밤 | 파워 | 2 | 턴 종료 시 손패가 0장이면 체력 1 회복, 방어도 3 획득. 강화: 회복 2, 방어도 4. |
...
보호의 빛: 체력이 50% 이하일 때 스킬 카드를 사용하면 체력 1 회복, 방어도 3 획득. 턴당 1회, 전투당 2회.
...
| 카드 | 타입 | 비용 | 효과 |
|---|---|---:|---|
| 성광 | 공격 | 1 | 피해 5. 체력 1 회복. 강화: 피해 7, 회복 2. |
| 심판의 종 | 공격 | 2 | 피해 13. 취약 상태의 적에게 피해 +5. 강화: 피해 17. |
| 보호 기도 | 스킬 | 1 | 방어도 8. 체력 1 회복. 강화: 방어도 11, 회복 2. |
| 정화 | 스킬 | 1 | 약화/취약/부상 중 하나 제거. 카드 1장 드로우. 강화: 비용 0. |
| 빛의 장막 | 스킬 | 2 | 방어도 14. 다음 턴 시작 시 체력 3 회복. 강화: 방어도 18, 회복 4. |
| 성수 한 방울 | 스킬 | 0 | 체력 1 회복. 이번 턴 다음 회복량 +20%. 강화: 회복 2. |
...
```txt
기본: 전투 승리 시 판돈 카운터 +1. 현재 체력 70% 이상으로 승리하면 +2로 계산. 엘리트/보스 승리도 +2로 계산. 한 전투 최대 +2.
판돈 카운터가 5 이상이면 전투 보상 후 판돈 보상을 굴리고 카운터를 0으로 리셋.
유대 60: 낮은 보상 확률 -10%, 중간 보상 확률 +7%, 높은 보상 확률 +3%.
유대 100: 판돈 보상 발생 시 보상 2개를 굴려 1개를 선택.
```

보상형 동료는 전투 중 패시브가 없다. 기본 공격은 하지만, 평균 전투 성능은 다른 동료보다 낮아야 한다. 카일은 대박이 터지면 런을 쉽게 만들 수 있지만, 보통 경로의 기대 성능은 평균 동료보다 낮게 시작한다.

판돈 보상 테이블 초안:

```txt
낮은 보상 55%: 골드 20~35 또는 카드 보상 스킵 골드 +8 1회.
중간 보상 30%: 골드 55~75, 상점 서비스 15% 할인권, 일반 장비 후보.
높은 보상 12%: 희귀 카드 후보, 비범 장비 후보, 주인공 카드 강화.
대박 3%: 희귀 장비 후보 또는 골드 150.
```

평균 기대값은 일반 동료의 전투 기여보다 낮게 잡는다. 대신 체력 70% 이상 승리, 엘리트 도전, 이벤트 선택과 결합하면 도박사 플레이가 분명히 보상받아야 한다.

### 서약 전술 3개

```txt
계산된 모험: 전투에서 골드를 소비하는 카드를 사용하면 방어도 6 획득. 턴당 1회.
행운의 장부: 판돈 보상에서 낮은 보상이 나오면 골드 10을 추가로 얻는다.
...
| 카드 | 타입 | 비용 | 효과 |
|---|---|---:|---|
| 동전 던지기 | 공격 | 0 | 피해 3. 이번 전투에서 카일 판돈 카운터가 오르면 골드 3 추가. 강화: 피해 5. |
...
| 보상 예감 | 스킬 | 1 | 이번 전투 승리 시 판돈 카운터 +1 추가. 한 전투 최대 +2 규칙은 유지. 강화: 방어도 4 추가. |
| 장사꾼의 감 | 파워 | 1 | 판돈 보상에서 낮은 보상이 나올 확률 -5%, 중간 보상 확률 +5%. 강화: 낮은 -8%, 중간 +8%. |
...
| 마렌 | 약한 회복, 피해 감소, 방어 | 순수 암살 피해 강화 |
| 토르 | 첫 피해 감소, 약화, 큰 방어 | 드로우 전용 장비 |
| 리나 | 독, 약화, 취약, 디버프 증폭 | 순수 방어 장비 |
| 노아 | 드로우, 비용 감소, 손패 보존 | 회복량 증가 |
| 이솔 | 약한 회복, 보호, 위기 회복 | 독 증폭 |
| 카일 | 판돈 보상, 상점 할인, 보상 확률 | 기본 공격 피해 대폭 강화 |
...
전술 정비 1장: 초반부터 드로우와 전술 표식의 재미를 보여주되, 덱을 과하게 빠르게 만들지 않는다.
```
...
| 전술 정비 | 스킬 | 0 | 카드 1장 드로우. 전술 표식 1 부여 | 카드 2장 드로우. 전술 표식 1 부여 |
...
| p_tactical_prep | 전술 정비 | 0 | starter | 카드 1장 드로우. 전술 표식 1. 강화: 카드 2장 드로우. |
...
공통 이벤트 10개
Act 1 이벤트 4개
Act 1 동료 흔적 이벤트 3개 이상
Act 1 중간 보스/계약 노드 1개
여관 이벤트 결과 9개
보물/특수보상 이벤트 6개
```
...
## 5.1 Act 1 동료 흔적 이벤트와 계약 노드

Act 1 초반에는 동료 흔적 이벤트로 후보를 예고할 수 있다. 다만 핵심 동료 시스템은 Act 1 중간 보스/계약 노드에서 실제로 열린다.

공통 규칙:

```txt
Act 1 depth 2~5 사이에 0~1회 노출 권장.
동료를 영입하지 않는다.
동료 카드를 덱에 추가하지 않는다.
동료 슬롯을 차지하지 않는다.
다음 전투 1회성 보정 또는 정보 보상을 제공한다.
Act 1 중간 보스 후 첫 동료 후보에는 예고로 본 동료가 최소 1명 포함될 수 있다.
```
...
선택지:
1. 흔적을 따라간다.
   - 로완이 Act 1 중간 보스 후 후보에 등장할 확률 크게 증가.
...
선택지:
1. 암호를 해독한다.
   - 세라가 Act 1 중간 보스 후 후보에 등장할 확률 크게 증가.
   - 다음 전투 첫 공격 카드 피해 +3.
...
선택지:
1. 증표를 정비한다.
   - 엘드릭이 Act 1 중간 보스 후 후보에 등장할 확률 크게 증가.
   - 다음 전투 시작 방어도 5.
...
위치: Act 1 depth 6 고정.
전투 성격: 강한 엘리트급 중간 보스.
승리 보상: 첫 동료 3택, 서약 전술 3택, 동료 카드 3장 중 2장 선택.
서사 표현: 동료와 이번 여정의 증표를 함께 찍는 장면.
```

중간 보스는 동료 없는 초반 덱을 너무 오래 끌고 가지 않기 위한 장치다. 전투가 길어지는 보스가 아니라, Act 1 초반 학습을 마친 뒤 동료 시스템을 여는 문이어야 한다.
...
위치: Act 2 depth 6 고정.
전투 없음.
선택지: 주인공 강화 / 동료 1 강화 / 동료 2 강화.
주인공 강화: 강화 후보 2개 중 1개 선택.
동료 강화: 해당 동료 유대 +10, 보유한 해당 동료 카드 1장 강화 후보.
서약 전술 강화 없음.
```
...
전투 중 회복 카드는 여관을 대체하면 안 된다. 여관은 큰 체력 회복과 다음 경로 계산을 담당하고, 회복 카드는 전투 중 작은 안정성이나 위기 대응을 담당한다.

```txt
일반 여관: 확실한 회복.
이벤트 여관: 수상하지만 긍정적 변동성이 있는 회복/기회.
회복 카드: 약한 회복 + 방어/정화/피해 감소/조건부 보상.
치유 감소 적: Act 2부터 일부 등장해 회복 덱이 무조건 해답이 되는 것을 막음.
```

치유 감소는 이벤트 손해가 아니라 전투 의도다.

```txt
치유 감소 수치: 받는 회복량 -50%.
지속: 1~2턴.
표시: 적 의도와 상태 아이콘에서 미리 보임.
Act 1: 거의 사용하지 않음.
Act 2: 학회/상단 변이체 일부가 사용.
Act 3: 코어/이계 적이 더 자주 사용.
```
...
우리 게임은 4에너지/6드로우라 턴당 선택량이 크고, Act 1 중반부터 첫 동료와 서약 전술이 붙는다.
따라서 Act 1 중간 보스는 80~105 HP, Act 1 보스는 첫 동료 보유 기준 220~270 HP에서 출발한다.
다만 난이도는 적 체력만 올려 맞추지 않고, 카드 비용 분포와 적 의도 패턴을 함께 조정한다.
```
...
5. Act 2 강화 타이밍에서 주인공을 밀 것인가, 특정 동료를 밀 것인가?
```
...
첫 동료, 집중 투자:
- Act 2 중간 강화 노드와 Act 2 보스 후 동료 강화를 선택하면 60점 안정 도달.
- Act 3 후반에 100점 가능.
...
- Act 3 중반 50~70점.
- Act 3 후반 70~90점.
- 집중 투자하면 100점 가능하지만 두 동료 모두 100점은 드물어야 한다.
```
...
### 3.4 Act 1 중간 보스와 첫 동료 영입

동료가 Act 1 보스 후에야 정식 영입되면, 게임의 차별점이 너무 늦게 드러난다. 따라서 첫 동료는 Act 1 중간 지점의 필수 노드에서 연다.

구조:

```txt
Act 1 depth 1~5: 주인공 단독 학습 구간.
Act 1 depth 6: 중간 보스/계약 노드.
중간 보스 승리 후: 첫 동료 3택, 서약 전술 선택, 동료 카드 2장 획득.
Act 1 depth 7~12: 첫 동료와 서약 전술을 실제 전투에서 사용.
Act 1 보스 승리 후: 두 번째 동료 3택.
```

Act 1 초반 동료 흔적 이벤트는 선택 사항으로 유지한다.

```txt
Act 1 depth 2~5 사이에 동료 흔적 이벤트 0~1회.
흔적으로 본 후보는 중간 보스 후 3택에 등장할 확률 증가.
보상은 다음 전투 1회성 보정 또는 정보 제공으로 제한.
```
...
HP 80~105.
전투 턴 수 5~7턴.
일반 엘리트보다 보상은 크지만, Act 보스보다 짧다.
동료 없는 구간을 더 늘리는 장기전 패턴은 금지.
```
...
기본값은 에너지 4, 드로우 6이므로 일반적인 3에너지/5드로우 덱빌더보다 플레이어의 선택 폭이 넓다. 다만 이것은 손패 대부분을 매 턴 전부 쓰게 하려는 구조가 아니다. 카드 비용 분포를 조금 무겁게 잡아, 플레이어가 6장 중 3~4장을 고르는 게임으로 만든다.

```txt
Act 1 초반 주인공 평균 카드 출력: 턴당 피해 12~22 또는 방어도 10~18.
Act 1 후반 첫 동료 보유 시 평균 출력: 턴당 피해 24~38 또는 방어도 16~26.
Act 2 동료 2명 보유 시 동료 기여: 전체 피해의 20~32%.
Act 3 동료 2명 보유 시 동료 기여: 전체 피해의 22~35%.
```
...
Slay the Spire의 Act 1 보스가 3에너지/5드로우 기준 140~250 HP임을 감안하면, 본 게임의 Act 1 보스는 첫 동료 보유 기준 220~270 HP에서 출발한다. 단, 추가 변동성 도구가 적으므로 체력과 공격 의도를 동시에 크게 올리지는 않는다.

난이도 조정 순서:

```txt
1. 카드 비용과 드로우/에너지 카드의 효율을 먼저 본다.
2. 적의 의도 패턴과 치유 감소/방어 파괴 같은 대응 요구를 조정한다.
3. 마지막으로 적 체력을 5~10% 단위로 보정한다.
```
...
Act 1 중간 보스: 첫 동료 3택.
Act 1 보스: 두 번째 동료 3택.
Act 2 중간 강화 노드: 주인공/동료 강화 3택.
Act 2 보스: 주인공/동료 대강화 3택.
Act 3 보스: 엔딩.
```
...
Act 1 초반의 동료 흔적 이벤트는 후보 예고용이다. 게임의 핵심 동료 시스템은 Act 1 depth 6 중간 보스 후 실제 영입으로 열린다.

---

## 8. 카드 수치 기준

4에너지/6드로우 환경에서는 낮은 비용 카드와 드로우 카드가 쉽게 폭주한다. 따라서 "선택지가 많다"와 "카드를 전부 쓴다"를 분리한다.
...
3비용 또는 X비용 카드: 8~12%.
보통 턴 사용 카드 수: 3~4장.
손패 6장 중 1~2장이 남아도 정상.
```

고비용 카드 원칙:

```txt
2비용 카드는 한 턴의 중심 행동이어야 한다.
3비용 카드는 사용 턴의 방어/후속 행동을 포기하는 위험이 있어야 한다.
드로우 2장 이상, 에너지 획득, 비용 0화는 1비용 이하에 과하게 몰지 않는다.
조건을 만족한 동료 카드는 같은 비용 주인공 카드보다 110~125% 효율까지 허용한다.
조건을 만족하지 못한 동료 카드는 80~95% 효율에 머물러야 한다.
```
...
회복은 방어, 정화, 피해 감소, 흡혈 공격, 체력 손실 대가와 결합할 때 가장 좋다.
순수 회복량이 여관 회복을 대체하면 안 된다.
```
...
카일은 고정 골드 보상 동료가 아니다. 매 전투 승리마다 보상을 주면 평균 경제가 쉽게 깨지고, 상점/여관 선택 압박이 사라진다.

카일의 핵심은 **5승 판돈 보상**이다.

```txt
전투 승리: 판돈 카운터 +1.
현재 체력 70% 이상으로 승리: +2로 계산.
엘리트/보스 승리: +2로 계산.
한 전투 최대 증가량: +2.
판돈 카운터가 5 이상이면 전투 보상 후 판돈 보상을 굴리고 0으로 리셋.
```
...
```txt
유대 60: 낮은 보상 -10%, 중간 +7%, 높은 +3%.
유대 100: 판돈 보상 2개를 굴려 1개 선택.
```

의도:

```txt
평균 성능은 일반 전투 동료보다 낮다.
체력 70% 이상 승리와 엘리트 도전을 노리면 보상 속도가 빨라진다.
대박이 터지면 그 런은 쉬워져도 된다.
다만 대박을 기대하지 않는 보통 경로는 평균보다 약해야 한다.
```

카일 플레이는 "경제가 항상 좋다"가 아니라 "좋은 흐름을 만들면 판돈이 빨리 찬다"여야 한다.

---

## 11. 회복과 치유 감소 밸런스

회복 카드는 여관의 역할을 빼앗으면 안 된다. 전투 중 회복은 방어 실패를 무한히 되돌리는 수단이 아니라, 위기 대응 또는 특정 캐릭터의 전술 정체성이다.
...
```txt
효과: 1~2턴 동안 받는 회복량 -50%.
Act 1: 일반 적은 사용하지 않음. 특수 패턴이나 보스 예고용만 가능.
Act 2: 학회/상단 변이체 일부가 예고 후 사용.
Act 3: 코어/이계 적이 회복 감소와 타이밍 방해를 더 자주 사용.
```

치유 감소는 힐러를 죽이는 장치가 아니라, 회복 덱이 방어/정화/공격 타이밍도 함께 보게 만드는 장치다.
...
```txt
MVP 첫 10회 내부 런에서 Act 1 중간 보스 도달률 70~85%.
Act 1 중간 보스 처치율 55~75%.
Act 1 보스 도달률 45~65%.
Act 1 보스 처치율 30~50%.
카드 보상 스킵률 10~25%.
엘리트 선택 경로 진입률 25~45%.
