...
별도 아트 문서는 `../art_director_game_design_brief.md`와 `../art_director_worldbuilding_brief.md`를 기준으로 한다. 세계관은 스토리텔링형 컷신보다 증표, 변이체, 버려진 고대 성, 이벤트 문구, UI 소품으로 전달한다.
...
Act당 보스 수: 1종
Act 1 보스 후: 동료 영입
Act 2 보스 후: 동료 영입 또는 기존 동료 유대 보상
Act 3 보스 후: 엔딩
```
...
Act 2: 12노드 → Act 2 보스 → 동료 선택/유대 보상
...
```txt
유대 점수 범위: 동료별 0~100
Act 1 보스 후 신규 영입 동료: 0점 시작
Act 2 보스 후 신규 영입 동료: 20점 시작
Act 2 보스 후 기존 동료 선택: 해당 동료 +20점
보너스 구간: 30 / 60 / 100
최대값: 100, 초과 점수는 버림
```
...
두 번째 동료 보통 경로: Act 3 중반 30점, Act 3 후반 50~70점
두 번째 동료 100점: 매우 드묾
```
...
기존 동료가 재등장하면 3택 중 하나가 기존 동료 유대 보상이 된다.

```txt
25%:
[기존 동료 유대 +20] [신규 동료 B] [신규 동료 C]

75%:
[신규 동료 B] [신규 동료 C] [신규 동료 D]
```

기존 동료를 선택해도 중복 영입은 되지 않는다. 해당 동료의 유대 점수가 증가한다.

기존 동료 유대 보상 선택 시:

```txt
해당 동료 유대 점수 +20
유대 점수가 30/60/100 구간을 넘으면 즉시 보너스 적용
유대 점수는 100을 넘지 않음
선택한 서약 전술은 변경하지 않음
```
...
3. 해당 동료의 카드 8장 중 3장을 표시한다.
4. 플레이어는 카드 3장 중 2장을 선택한다.
5. 선택한 2장은 즉시 덱에 추가된다.
6. 선택하지 않은 1장과 미공개 5장은 이후 카드 보상 후보가 된다.
```
...
```txt
로완 - 붉은 추격: 한 턴에 같은 적을 2회 이상 공격하면 로완이 전술 표식 대상을 서약 공격.
세라 - 검은매 연계: 한 턴에 비용 0 카드를 2장 이상 사용하면 세라가 피해 4의 서약 공격을 하고 카드 1장 드로우.
엘드릭 - 철벽 반격: 한 턴에 방어도 15 이상 획득하면 방어도 일부를 피해로 전환.
```
...
4에너지/6드로우 구조라 일반적인 3에너지/5드로우 덱빌더보다 플레이어의 초반 처리량이 높다. 대신 추가 변동성 도구가 적으므로 HP를 올리되, 공격 의도는 플레이테스트 전까지 보수적으로 유지한다.
...
Act 2 보스 후 새 동료와 기존 동료 유대 보상 선택의 무게를 만든다.
...
Act 1 보스는 02_systems_spec.md의 "검은 지문을 가진 용병대장" 초안을 사용한다.
...
세계관은 스토리텔링형 진행이 아니라 시스템과 아트를 묶는 기준이다. 마법 재난, 변이체, 증표, 검은 지문, 버려진 고대 성의 코어는 카드/동료/이벤트 선택을 선명하게 만들 때만 전면에 둔다.
...
```txt
레퍼런스의 일부 시스템은 본 게임에 직접 가져오지 않는다.
그 빈자리는 동료, 서약 전술, 유대 점수, 장비, 이벤트, 여관이 나눠 맡는다.
```
...
대신 추가 변동성 도구와 회복/강화 구조가 함께 존재한다.
우리 게임은 4에너지/6드로우라 턴당 선택량이 크지만, Act 1 보스 후 동료와 서약 전술이 붙는다.
...
서약 전술은 본 게임만의 핵심 전투 정체성이다. 동료를 영입하면 해당 동료의 서약 전술 3개 중 1개를 선택한다.

```txt
동료마다 서약 전술 3개.
영입 시 3개 중 1개 선택.
선택한 서약 전술은 런 동안 고정.
서약 전술은 전투마다 랜덤 지정되지 않음.
서약 전술은 업그레이드되지 않음.
```

서약 전술은 플레이어가 특정 전투 리듬을 만들 때 발동하는 합동 규칙이다.

```txt
로완: 같은 적을 반복 공격하거나 전술 표식을 유지해 약한 서약 공격을 만든다.
세라: 비용 0 카드와 연타로 템포를 만들되, 추가 공격 피해는 낮게 둔다.
엘드릭: 방어도 기준을 넘겨 반격한다.
브람: 광역 피해를 이어 폭발을 만든다.
```

서약 전술은 동료의 성격과 덱 방향을 알려주는 고정 규칙이다. 랜덤 버프가 아니라, 플레이어가 선택한 이번 런의 약속이어야 한다.

### 3.2.1 서약 전술 밸런스 예산

서약 전술은 동료 시스템의 얼굴이지만, 전투의 주 조작권은 여전히 카드에 있어야 한다. 따라서 서약 전술의 기대값은 **카드 0.5~1장 가치**를 넘지 않게 시작한다.
...
전투당 1회 공격형 서약: 피해 8~14 또는 동료 기본 공격 1회.
방어형 서약: 방어도 4~6.
드로우 서약: 카드 1장. 반드시 처치/타입 조합/조건부 회복 같은 명확한 조건 필요.
에너지/비용 감소 서약: 전투당 1회가 기본. 반복 발동 금지.
회복 서약: 체력 2~3, 전투당 2회 이하 권장.
골드 서약: 일반 전투 1회당 기대 골드 +3~6 이내.
```
...
```txt
조건을 의식하지 않는 플레이: 일반 전투당 0~1회.
조건을 의식한 평범한 덱: 일반 전투당 1~2회.
해당 동료 카드/장비를 집중한 덱: 일반 전투당 2~3회.
전투당 4회 이상 자주 발동하면 조건을 어렵게 하거나 효과를 낮춘다.
```

공격형 서약 전술의 추가 공격은 기본적으로 **서약 공격**으로 처리한다.

```txt
서약 공격 = 동료 기본 공격과 별도인 약한 공격.
기본값: 동료 기본 공격력의 75%, 소수점 버림, 최소 피해 3.
패시브 추가 피해와 장비의 동료 기본 공격 피해 보너스는 적용하지 않는다.
전술 표식 대상 우선 규칙은 따른다.
```

이렇게 하면 로완 같은 공격형 동료도 서약 전술이 강하게 느껴지지만, 동료 자동 공격이 플레이어 카드보다 앞서지 않는다.

### 3.3 유대 점수

유대는 단계가 아니라 0~100점으로 쌓이는 동료별 점수다.
...
```txt
30점: 친숙. 해당 동료 기본 공격력 +1, 대사/전투 UI 변화 가능.
60점: 신뢰. 해당 동료 패시브 수치 소폭 강화.
100점: 서약 완성. 해당 동료별 작은 완성 보너스와 런 결과 문구 가능.
```

유대 점수 보너스는 미미해야 한다. 목적은 전투력 폭발이 아니라, "이 동료와 여정을 함께했다"는 감각이다.

### 3.3.1 유대 점수 페이스

유대 점수는 자동으로 오르지만, 100점은 의식적으로 투자해야 도달하는 값이어야 한다.
...
첫 동료, 집중 투자:
- Act 2 보스 후 기존 동료 선택 또는 개인 이벤트 선택 시 60점 안정 도달.
- Act 3 후반에 100점 가능.

두 번째 동료:
- Act 2 보스 후 신규 영입 시 20점 시작.
- Act 3 중반 30점 도달.
- Act 3 후반 50~70점.
- 100점은 매우 드물어야 한다.
```
...
```txt
30점 보너스: 전투당 피해 3~5 또는 방어도 3~5 수준.
60점 보너스: 패시브 수치 30~60% 상승. 단, 반복형 패시브는 +1~2만 상승.
100점 보너스: 전투당 1회 작은 보너스 또는 경제/엔딩 문구 보너스.
```

유대 점수는 100을 넘지 않는다. 여러 보상으로 구간을 한 번에 넘으면 새 구간 보너스만 적용하고 초과 점수는 버린다.
...
동료 기여에는 기본 공격, 패시브, 서약 전술 피해, 유대 보너스 피해를 모두 포함한다. 동료 기여가 낮으면 동료가 장식처럼 느껴진다. 동료 기여가 40%를 넘으면 플레이어 카드 선택보다 자동 공격이 중심이 된다.
...
Slay the Spire의 Act 1 보스가 3에너지/5드로우 기준 140~250 HP임을 감안하면, 본 게임의 4에너지/6드로우 기준 Act 1 보스는 190~240 HP에서 출발한다. 단, 추가 변동성 도구가 적으므로 체력만 올리고 공격 의도까지 동시에 올리지는 않는다.
...
Slay the Spire의 실제 상점 카드 가격은 일반 45~55, 비범 68~82, 희귀 135~165이다. 본 게임은 장비가 상점의 장기 성장축이므로, 카드는 비슷하게 두고 장비 가격을 카드보다 약간 무겁게 둔다.
...
첫 정식 버전에서는 한 번에 하나씩 불리한 조건을 더하는 **원정 규율**을 고려한다.
