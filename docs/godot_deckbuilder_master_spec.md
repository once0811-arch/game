...
```txt
로완 - 붉은 추격: 한 턴에 같은 적을 2회 이상 공격하면 로완이 추가 공격.
세라 - 검은매 연계: 한 턴에 비용 0 카드를 2장 이상 사용하면 세라가 추가 공격하고 카드 1장 드로우.
...
```txt
붉은 추격: 한 턴에 같은 적을 2회 이상 공격하면 로완이 전술 표식 대상을 추가 공격. 턴당 1회.
...
```txt
검은매 연계: 한 턴에 비용 0 카드를 2장 이상 사용하면 세라가 추가 공격하고 카드 1장 드로우. 턴당 1회.
...
```txt
연쇄 폭발: 한 턴에 광역 피해를 2회 이상 주면 모든 적에게 피해 3. 턴당 1회.
도화선 계산: 비용 2 이상 공격 카드를 사용하면 다음 광역 공격 비용 -1. 전투당 1회.
...
수호 맹세: 이번 턴 받는 피해를 2 이상 줄이면 카드 1장 드로우. 턴당 1회.
...
상처 봉합: 체력이 50% 이하일 때 회복 카드를 사용하면 추가로 체력 2 회복. 턴당 1회.
...
부식 표식: 전술 표식 대상에게 독이 있다면 리나가 추가 공격. 턴당 1회.
```
...
궤도 계산: 카드 3장 이상 드로우한 턴에 다음 카드 비용 -1. 턴당 1회.
```
...
보호의 빛: 체력이 50% 이하일 때 스킬 카드를 사용하면 체력 2 회복. 턴당 1회.
...
행운의 장부: 전투 승리 시 10% 특수보상이 발생하지 않았다면 추가 골드 5 획득.
위험한 흥정: 체력을 잃는 카드를 사용하면 카드 1장 드로우. 턴당 1회.
```
...
```txt
로완: 같은 적을 반복 공격하거나 전술 표식을 유지한다.
세라: 비용 0 카드와 연타로 템포를 만든다.
엘드릭: 방어도 기준을 넘겨 반격한다.
